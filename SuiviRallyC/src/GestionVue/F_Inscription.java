			...
			public void actionPerformed(ActionEvent e) {
				...
				for (EditionRallye era : lstEditionRallyes) {
					if(era.getAnneeER() == ery) {
						lblDate.setText(" De " + era.getDateDebER() + " Ã  " + era.getDateFinER() );
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (Coureur cr : lstCoureurs) {
					...
					if(textField.getText() == cr.getNomC() && textField_1.getText() == cr.getPrenomC() && textField_8.getText() == cr.getDateNaissC()
						...
						exist = true;
						System.out.println("DÂ¨Â¦ja inscrit!");
						break;
				...
				if (!exist) {
					...
					lstVehicules.add(vi);
					lstCoureurs.add(r);
					EditionRallye ref = null;
					for(EditionRallye er : lstEditionRallyes) {
						String nom = ""+(String) comboBox.getSelectedItem();
						String annee =""+comboBox_2.getSelectedItem();
						if(nom.equals(er.getNomR()) && 
								annee.equals(""+er.getAnneeER())) {
							ref = er;
							break;
						}
					}
